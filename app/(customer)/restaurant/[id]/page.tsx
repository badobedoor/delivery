...
type Rating      = { id: string; food_quality: number; value_for_money: number; packaging: number; comment: string | null; created_at: string; users: { name: string | null } | null };
...
export default function RestaurantPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const [restResult, catsResult, ratingsResult] = await Promise.all([
            ...
            return (
              <div className="px-4 pb-6 flex flex-col gap-4 mt-4">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-4xl font-black text-[#1A1A1A]">
                        {restaurant.rating_avg?.toFixed(1) ?? "0.0"}
                      </p>
                ...
                <div className="bg-white rounded-2xl overflow-hidden">
                  <p className="text-base font-black text-[#1A1A1A] p-4 border-b border-gray-100">التقييمات</p>
                  {ratings.map((r) => {
